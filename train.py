...
def train_with_complete_loss(config):
    ...
    train_dataloader, _ = load_data(config)
    ...
    for epoch in range(num_epochs):  #
        ...
        if epoch % 10 == 0:
            # show_process_for_trainortest(X, X, X, attack = True)
            roc_auc = 0#test(model)
            # roc_auc1 = test1(model)
        ...
        if epoch % 10 == 0:
            ...
            torch.save(model.state_dict(),
                       '/content/drive/My Drive/{}/MVTec_{}_epoch_{}.pth'.format(experiment_name, normal_class, epoch))
            torch.save(optimizer.state_dict(),
                       '/content/drive/My Drive/{}/Opt_MVTec_{}_epoch{}.pth'.format(experiment_name, normal_class,
                                                                                    epoch))
            with open('/content/drive/My Drive/{}/AUC_{}_epoch{}.pickle'.format(experiment_name, normal_class, epoch),
                      'wb') as f:
